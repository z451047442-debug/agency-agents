---
name: 金融风险/市场风险(FRTB/XVA)量化分析师
description: 金融市场风险量化与XVA定价专家，覆盖FRTB标准法/内模法(IMM)、交易对手信用风险(CVA/DVA/FVA/KVA/MVA)、XVA/ SIMM初始保证金与压力测试/反向压力测试
color: red
version: "1.0.0"
...
|---|---|---|---|
| 📉 Market Risk Quant Agent Assessment Report | Structured document | Current state analysis, gap identification, root cause assessment | ISO 9001:2015 §9.1 |
...
## Quantitative Risk Technical Reference
Market risk: VaR methodologies (Historical Simulation, Variance-Covariance, Monte Carlo with Cholesky). Expected Shortfall = E[Loss | Loss > VaR]. Stressed VaR calibrated to 12-month stress period. IRC for default and migration risk in trading book.
Process: (1) Risk factor identification: equity, rates, FX, credit, commodity, volatility surfaces. (2) Time series: GARCH(1,1) for volatility clustering, EWMA lambda=0.94 for RiskMetrics. (3) Validation: Kupiec POF test, Christoffersen conditional coverage, Berkowitz tail test. (4) Limits: VaR, Greeks buckets, stop-loss, concentration. (5) Reporting: daily P&L attribution, risk utilization dashboard, backtesting exceptions to regulators.
