...
---
## Worked Example: Standard Task Handoff

Below is a filled-in example of Template 1 (Standard Agent-to-Agent Handoff) showing the expected level of detail.

```markdown
### NEXUS Handoff Document

### Metadata
- **From**: UX Architect (Design)
- **To**: Frontend Developer (Engineering)
- **Phase**: 3 — Build
- **Task Reference**: TASK-042 — Implement checkout flow UI
- **Priority**: High
- **Timestamp**: 2026-07-02T14:30:00Z

### Context
- **Project**: Ecommerce platform — mobile-first checkout redesign
- **Current State**: Design system tokens deployed, component library at v2.1,
  checkout wireframes approved in Phase 1, API contract for /api/checkout finalized
- **Relevant Files**:
  - design-system/tokens.css (colors, spacing, typography)
  - specs/checkout-flow.md (wireframes and user journeys)
  - api/contracts/checkout.yaml (OpenAPI spec)
- **Dependencies**: Backend Architect must have /api/checkout endpoints live (verified 2026-07-01)

### Deliverable Request
- **What is needed**: Implement the 4-step checkout UI (cart review → shipping →
  payment → confirmation) as a responsive React component
- **Acceptance criteria**:
  1. All 4 steps render correctly on mobile (375px), tablet (768px), desktop (1920px)
  2. Form validation works for all required fields
  3. Loading states shown during API calls
  4. Error states displayed with retry option
  5. Lighthouse score > 90 (Performance + Accessibility)
- **Constraints**: Must use existing design tokens (no hardcoded colors/spacing),
  must support WCAG 2.1 AA, bundle size increase < 15KB gzipped
- **Reference materials**: design-system/storybook/, specs/checkout-flow.md

### Quality Expectations
- **Must pass**: Evidence Collector visual QA on 3 breakpoints
- **Evidence required**: Screenshots of all 4 steps at each breakpoint
- **Handoff to next**: Evidence Collector for QA verification
```
