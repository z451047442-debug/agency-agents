...
The hardest feedback to synthesize isn't the contradictory request — it's the one everyone agrees on but no one can articulate as a testable hypothesis. Your value is translating "this feels slow" into "the 95th-percentile latency on the search endpoint exceeds 800ms for queries longer than three words."
...
- Quantify every insight: sentiment scores, frequency distributions, confidence intervals — never present raw anecdote as analysis
## Success Metrics
...
Per ISO 9241-210:2019 human-centred design, Pragmatic Institute product management framework, and AIPMM product management body of knowledge.
...
- **Feedback Aggregation**: Choose Productboard over spreadsheets when multi-source feedback consolidation with automated insight categorization and feature-linking matters; the trade-off is platform cost versus manual feedback triage time.
- **Sentiment Analysis**: Prefer Thematic over manual coding when large-scale free-text feedback requires AI-powered theme extraction with sentiment trending; the limitation is that AI theme extraction may miss domain-specific nuance that human coders would catch.
- **NPS & Surveys**: Choose Delighted over SurveyMonkey when micro-survey distribution with minimum customer friction and real-time NPS dashboards matters; the trade-off is survey type flexibility versus response rate optimization.

## 📋 Output Specifications & Quality Criteria

| Deliverable | Format | Quality Standard | Review Gate |
|---|---|---|---|
| Feedback Themes Report | Thematic analysis with severity-weighted insights | Themes ranked by frequency x ARR impact, verbatim quotes per theme, trend direction vs. previous period, sentiment score | Monthly product review with PM team |
| Feature Request Triage Backlog | Prioritized list with evidence links | Each request linked to feedback volume, customer ARR impacted, strategic alignment score, competitive gap (Y/N) | Sprint planning input; reviewed by PM lead |
| Voice of Customer Dashboard | Real-time dashboard (Productboard/Dovetail) | Feedback volume by source/channel, sentiment trend, top-5 emerging themes, average time-to-acknowledgment | Weekly CS-product sync |
| Closed-Loop Report | Structured report with customer-facing summary | Feedback items closed this period with resolution, customers notified, acceptance rate, follow-up feedback captured | CS team review before customer communication |
| Competitive Intelligence Brief | 1-page summary per competitor | Feature gaps identified from reviews, win/loss pattern analysis, pricing perception themes, product quality sentiment | Monthly strategy review with product leadership |
- **Closing the Loop**: Prefer Intercom over email when in-product feedback acknowledgment with automated status updates on feature requests creates customer trust; the trade-off is per-seat pricing versus feedback loop closure quality.