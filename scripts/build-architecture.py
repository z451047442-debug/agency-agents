...
def collect_script_categories() -> dict:
    ...
    SCRIPT_MAP: dict[str, tuple[str, str]] = {
        ...
        "audit-security.py": ("quality", "Security audit: shell/Python/config pattern scanning"),
        "fix-lint.py": ("maintenance", "Incremental lint fixer: ruff + mypy auto-fix pipeline"),
        "extract-patterns.py": ("discovery", "Feedback pattern analysis: surface improvement insights"),
...
def collect_omc_artifacts() -> list[dict]:
    """Return the 4 OMC plugin artifacts generated by dedicated scripts."""
    return [
        {
            "name": "NEXUS Skills",
            "script": "generate-nexus-skills.py",
            "format": "SKILL.md",
            "count": "7 files",
            "desc": "按阶段关键词触发的 skill 文件，内含该阶段 agent 列表和质量门检查清单",
        },
        {
            "name": "模型路由",
            "script": "generate-omc-model-routing.py",
            "format": "model-routing.json",
            "count": "1399 条目",
            "desc": "按 agent 质量评分分配模型层级 — A级→Opus / B级→Sonnet / C/D级→Haiku",
        },
        {
            "name": "关键词 Hook",
            "script": "generate-omc-hooks.py",
            "format": "hooks.json",
            "count": "领域→agent",
            "desc": "从 agent 名称和描述中提取领域关键词，用户提及时自动推荐对应 agent",
        },
        {
            "name": "团队管线",
            "script": "generate-omc-team-config.py",
            "format": "team-config.json",
            "count": "5 阶段管线",
            "desc": "NEXUS 7阶段 → OMC 5阶段团队管线映射 (plan→prd→exec→verify→fix)",
        },
    ]
...
**{data['total_agents']:,} AI Agent Personality Definitions · {data['total_categories']} Categories · {sc['py_total']} Tooling Scripts · {data['total_tests']:,} Tests · OMC Smart Plugin**
...
{_fmt_table(["产物", "生成器", "输出", "规模", "功能"], [[a["name"], a["script"], a["format"], a["count"], a["desc"]] for a in data['omc_artifacts']])}
...
| OMC plugin artifacts | {len(data['omc_artifacts'])} |
...
def _omc_cards(data: dict) -> str:
    """Build OMC artifact cards HTML."""
    cards = []
    for a in data["omc_artifacts"]:
        cards.append(
            '<div><div class="card" style="border-top:3px solid #f59e0b">'
            f'<div class="t">{a["name"]}<br>'
            f'<span style="font-size:10px;color:#92400e">{a["format"]} &middot; {a["count"]}</span></div>'
            f'<div class="d" style="font-size:12px">{a["desc"]}<br>'
            f'<span style="font-size:10px;color:#666">{a["script"]}</span></div></div></div>'
        )
    return "\n".join(cards)
...
<div class="header">
  ...
  <p>{data['total_agents']:,} AI Agent 角色定义库 | {data['total_categories']} 行业分类 | {sc['py_total']} 工具脚本 | {data['total_tests']:,} 测试 | {data['integration_targets_count']} 集成目标 | OMC 智能插件 | Python {data['python_req']}</p>
</div>
<div class="container">
...
<div class="layer l5b">
  <div class="layer-h">第 5B 层: OMC 智能插件 (oh-my-claudecode · plugin 模式 · 4 个生成器)</div>
  <div class="layer-b">
    <div class="st" style="margin-bottom:14px">OMC 是唯一不通过 convert.py 转换的集成目标 — 4 个专用生成器将 agent 元数据转化为运行时智能</div>
    <div class="grid" style="grid-template-columns:repeat(4,1fr)">
{_omc_cards(data)}
    </div>
    <div class="st" style="margin-top:12px">与第 5 层的 10 个工具不同，OMC 不生成 agent 文件副本 — 而是产生模型路由表、关键词触发规则、阶段编排技能和团队管线配置，反向增强 Claude Code 自身智能</div>
  </div>
</div>
<div class="arr">&#8595;</div>
...
<div class="footer">
  ...
  架构: CI/CD -> 测试 -> 共享库 -> 工具脚本 -> Agent 内容 -> 集成目标 -> OMC 智能插件 -> 数据流 -> NEXUS 编排<br>
  生成时间: {data['generated']}
</div>
</body></html>"""
