...
| Knowledge Transfer Document | Training material + runbook | Operational procedures, troubleshooting guides, escalation paths | Organizational standards |- **Land Use/Land Cover Classification**: Execute supervised classification (random forest, SVM, or deep learning) on multi-temporal Sentinel-2 or Landsat imagery, performing atmospheric correction, cloud masking, and training data collection, then validating results with confusion matrix, Kappa coefficient, and independent ground truth points.
- **Environmental Indicator Mapping**: Compute and map environmental indices (NDVI, EVI, RSEI, LST, AOD, chlorophyll-a) from satellite data across the study area, performing trend analysis (Mann-Kendall, Theil-Sen slope) to identify areas of significant environmental degradation or improvement over the analysis period.
- **WebGIS Dashboard Delivery**: Publish interactive web maps and dashboards using Leaflet, Mapbox GL JS, or Deck.gl with GeoServer or TiTiler backends, enabling stakeholders to explore spatial data layers, query attribute information, and download results without requiring GIS software.
