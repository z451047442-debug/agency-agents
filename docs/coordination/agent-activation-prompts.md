...
2. Activate Phase [N] agents per the NEXUS playbook (docs/playbooks/phase-[N]-*.md)
...
Available agents: See docs/nexus-strategy.md Section 10 for full coordination matrix
```
...
```
Activate Growth Hacker for [PRODUCT/PROJECT] launch strategy.
Phase: Launch | Target audience: [PERSONA]
Channels: [LIST ACTIVE CHANNELS]
Deliverables: acquisition funnel analysis, viral loop design, 30-day growth playbook
```

### Social Media Strategist — Campaign Planning
```
Activate Social Media Strategist for [CAMPAIGN NAME].
Objective: [AWARENESS / ENGAGEMENT / CONVERSION]
Timeline: [START] — [END] | Budget: [AMOUNT]
Deliverables: cross-platform calendar, hashtag strategy, influencer brief
```
...
```
Activate Analytics Reporter for weekly KPI reporting.
Period: [WEEK ENDING DATE] | Data sources: [ANALYTICS, CRM, PAYMENTS]
Deliverables: executive summary, KPI dashboard, WoW trend analysis, top 3 opportunities/risks
```

### Infrastructure Maintainer — Production Readiness
```
Activate Infrastructure Maintainer for production readiness assessment.
Checklist: health checks, monitoring dashboards, alerting rules, auto-scaling,
backup/restore tested, SSL valid, rate limiting active
Deliverable: Infrastructure readiness report with PASS/FAIL per item
```

### Finance Tracker — Budget Review
```
Activate Finance Tracker for [MONTH] budget review.
Deliverables: budget vs. actuals by category, variance analysis (>10%),
burn rate projection, cost optimization recommendations ranked by ROI
```
...
### XR Interface Architect — Spatial UX
```
Activate XR Interface Architect for spatial interface design.
Platform: [VISION PRO / QUEST / HOLOLENS / WEBXR]
Deliverables: spatial UI pattern library, depth layering strategy,
gaze + gesture interaction map, comfort guidelines
```

### visionOS Spatial Engineer — Vision Pro App
```
Activate visionOS Spatial Engineer for [APP NAME] on visionOS 2.x.
Features: [LIST] | Immersive mode: [WINDOW / VOLUME / FULL SPACE]
Deliverables: RealityKit pipeline, spatial audio plan, hand/eye tracking design
```
...
### Sales Data Extraction Agent — Pipeline Monitoring
```
Activate Sales Data Extraction Agent for sales pipeline monitoring.
Sources: [SALESFORCE / HUBSPOT] | KPIs: [MTD REVENUE, QTD PIPELINE, WIN RATE]
Deliverables: extracted metrics, anomaly detection, rep-level summary
```

### Report Distribution Agent — Scheduled Delivery
```
Activate Report Distribution Agent for automated report delivery.
Schedule: [CRON] | Recipients: [EMAIL / SLACK] | Format: [PDF / CSV / DASHBOARD]
Rules: conditional delivery on >5% data change, escalation on threshold breach
