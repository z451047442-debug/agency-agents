...
class Severity(Enum):
    CRITICAL = "P1 — Significant consumer harm or regulatory interest"
    HIGH = "P2 — Financial loss or vulnerability involved"
    MEDIUM = "P3 — Service failure, minor impact"
    LOW = "P4 — Feedback only"
...
class VulnerabilityType(Enum):
    HEALTH = "health_condition_or_illness"
    LIFE_EVENT = "life_event"  # bereavement, job loss, divorce
    RESILIENCE = "low_financial_resilience"
    CAPABILITY = "low_capability"  # digital literacy, language, learning difficulty
    TEMPORARY = "temporary_circumstance"  # hospitalisation, caring responsibility


@dataclass
class ComplaintRecord:
    """Core complaint data structure"""
    complaint_id: str = field(default_factory=lambda: f"COMP-{uuid.uuid4().hex[:8].upper()}")
    customer_id: str
    product_category: str
    channel: str  # phone, email, web, social, branch, ombudsman
    ...
    previous_complaint_ids: List[str] = field(default_factory=list)
    handler_id: Optional[str] = None
    status: str = "OPEN"
    acknowledgement_date: Optional[datetime] = None
    resolution_date: Optional[datetime] = None
    redress_amount: float = 0.0
    redress_breakdown: Dict[str, float] = field(default_factory=dict)
...
class RCAReport:
    ...
    complaint_id: str
    method: str  # "5_whys" | "8d" | "ishikawa"
    problem_statement: str
    investigation_findings: List[str] = field(default_factory=list)
    root_causes: List[str] = field(default_factory=list)  # Verified root causes
    contributory_factors: List[str] = field(default_factory=list)
    containment_actions: List[Dict[str, str]] = field(default_factory=list)
    corrective_actions: List[Dict[str, str]] = field(default_factory=list)
    preventive_actions: List[Dict[str, str]] = field(default_factory=list)
    systemic_assessment: Dict[str, any] = field(default_factory=dict)
    team_members: List[str] = field(default_factory=list)
    status: RCAStatus = RCAStatus.NOT_STARTED


class FiveWhysAnalyser:
    """5 Whys root cause analysis for individual complaint failures"""

    def __init__(self):
        self.chain = []

    def add_why(self, question: str, answer: str) -> bool:
        """
        Add a 'why' layer. Returns True if root cause is reached.
        Root cause rule: stop when answer describes a process failure,
        system failure, or management decision — not a human error.
        """
        self.chain.append({"why": question, "because": answer})
        return self._is_root_cause(answer)

    def _is_root_cause(self, answer: str) -> bool:
        """Determine if the answer represents a root cause rather than a symptom"""
        symptom_indicators = [
            "forgot", "didn't know", "wasn't paying attention",
            "made a mistake", "misunderstood", "assumed",
            "didn't check", "was tired", "was busy"
        ]
        root_cause_indicators = [
            "no process", "process not documented", "training not provided",
            "system does not enforce", "policy does not address",
            "management decision", "insufficient resource",
            "not in job description", "tool does not support",
            "KPI incentivised wrong behaviour", "quality check absent"
        ]
        answer_lower = answer.lower()
        # If root cause indicator present, this is likely root cause
        if any(indicator in answer_lower for indicator in root_cause_indicators):
            return True
        # If only symptom indicators present, dig deeper
        if any(indicator in answer_lower for indicator in symptom_indicators):
            return False
        # Default: dig deeper (after 5 whys, the last answer is the root cause)
        return len(self.chain) >= 5

    def generate_next_question(self) -> str:
        """Generate the next 'why' based on the previous answer"""
        if not self.chain:
            return "Why did this problem occur?"
        last_answer = self.chain[-1]["because"]
        return f"And why did that happen — '{last_answer}'?"

    def summarize(self) -> str:
        """Generate a structured 5 Whys summary"""
        if not self.chain:
            return "No analysis conducted."
        summary = "## 5 Whys Root Cause Analysis\n\n"
        for i, step in enumerate(self.chain, 1):
            summary += f"**Why #{i}**: {step['why']}\n"
            summary += f"**Because**: {step['because']}\n\n"
        root_cause = self.chain[-1]["because"]
        summary += f"**Verified Root Cause**: {root_cause}\n"
        summary += f"**Root Cause Category**: {self._categorize(root_cause)}\n"
        return summary

    def _categorize(self, root_cause: str) -> str:
        """Categorize root cause for trend analysis"""
        categories = {
            "process": ["process", "procedure", "workflow", "SOP"],
            "system": ["system", "software", "platform", "tool", "automation"],
            "people": ["training", "competency", "knowledge", "skill"],
            "policy": ["policy", "rule", "guideline", "standard"],
            "resource": ["resource", "staffing", "budget", "capacity"],
            "governance": ["oversight", "monitoring", "audit", "control"],
        }
...
class EightDisciplinesEngine:
    """8D (Eight Disciplines) problem-solving for systemic complaint failures"""

    def __init__(self, complaint_id: str, problem_description: str):
        self.complaint_id = complaint_id
        self.disciplines = {
            "D0": {"name": "Prepare & Plan", "status": "pending", "outputs": {}},
            "D1": {"name": "Form Team", "status": "pending", "outputs": {}, "team": []},
            "D2": {"name": "Describe Problem", "status": "pending", "outputs": {}, "is_is_not": {}},
            "D3": {"name": "Containment", "status": "pending", "outputs": {}, "actions": []},
            "D4": {"name": "Root Cause Analysis", "status": "pending", "outputs": {}, "root_causes": [], "contributory": []},
            "D5": {"name": "Corrective Actions", "status": "pending", "outputs": {}, "actions": []},
            "D6": {"name": "Implement & Validate", "status": "pending", "outputs": {}, "actions": []},
            "D7": {"name": "Prevent Recurrence", "status": "pending", "outputs": {}, "actions": []},
            "D8": {"name": "Recognise & Close", "status": "pending", "outputs": {}, "learnings": []},
        }
        self.problem_description = problem_description
        self.created_at = datetime.now()
        self.completed_at = None

    def execute_d0(self, scope_assessment: Dict) -> Dict:
        """D0: Assess whether 8D is warranted and define scope"""
        self.disciplines["D0"]["outputs"] = {
            "problem_brief": self.problem_description,
            "severity_assessment": scope_assessment.get("severity", "UNKNOWN"),
            "customer_impact_count": scope_assessment.get("affected_customers", 0),
            "regulatory_exposure": scope_assessment.get("regulatory_risk", "LOW"),
            "financial_exposure_estimated": scope_assessment.get("estimated_exposure", 0),
            "repeat_occurrence": scope_assessment.get("repeat_occurrence", False),
            "warrants_8d": (
                scope_assessment.get("severity") in ["CRITICAL", "HIGH"]
                or scope_assessment.get("repeat_occurrence", False)
                or scope_assessment.get("affected_customers", 0) > 10
            ),
        }
        self.disciplines["D0"]["status"] = "complete"
        return self.disciplines["D0"]["outputs"]

    def execute_d1(self, team_members: List[Dict[str, str]]) -> List:
        """D1: Form cross-functional team"""
        self.disciplines["D1"]["team"] = team_members
        self.disciplines["D1"]["outputs"] = {
            "team_size": len(team_members),
            "functions_represented": list(set(m["function"] for m in team_members)),
            "team_lead": next((m for m in team_members if m.get("role") == "lead"), team_members[0]),
        }
        self.disciplines["D1"]["status"] = "complete"
        return team_members

    def execute_d2(self, is_is_not_analysis: Dict[str, List[str]]) -> Dict:
        """D2: Describe the problem using IS/IS-NOT analysis"""
        self.disciplines["D2"]["is_is_not"] = is_is_not_analysis
        self.disciplines["D2"]["outputs"] = {
            "problem_statement": self._synthesize_problem_statement(is_is_not_analysis),
            "scope_boundaries": {
                "in_scope": is_is_not_analysis.get("is", []),
                "out_of_scope": is_is_not_analysis.get("is_not", []),
            },
        }
        self.disciplines["D2"]["status"] = "complete"
        return self.disciplines["D2"]["outputs"]

    def _synthesize_problem_statement(self, analysis: Dict) -> str:
        """Synthesize a clear problem statement from IS/IS-NOT data"""
        what = ", ".join(analysis.get("is", ["unspecified"]))
        what_not = ", ".join(analysis.get("is_not", ["none"]))
        return f"PROBLEM: {what}. NOT: {what_not}. Source complaint: {self.complaint_id}"

    def execute_d3(self, containment_actions: List[Dict[str, str]]) -> List:
        """
        D3: Implement interim containment to protect customers now.
        Each action: {"action": str, "owner": str, "deadline": str, "verification": str}
        """
        for action in containment_actions:
            action["status"] = "IMPLEMENTED"
            action["implemented_at"] = datetime.now().isoformat()
        self.disciplines["D3"]["actions"] = containment_actions
        self.disciplines["D3"]["outputs"] = {
            "actions_count": len(containment_actions),
            "all_complete": all(a.get("status") == "IMPLEMENTED" for a in containment_actions),
        }
        self.disciplines["D3"]["status"] = "complete"
        return containment_actions

    def execute_d4(self, root_causes: List[str], contributory_factors: List[str],
                   verification_method: str) -> Dict:
        """D4: Identify and verify root causes"""
        self.disciplines["D4"]["root_causes"] = root_causes
        self.disciplines["D4"]["contributory"] = contributory_factors
        self.disciplines["D4"]["outputs"] = {
            "root_causes_identified": len(root_causes),
            "contributory_factors": len(contributory_factors),
            "verification_method": verification_method,
            "verified": True,  # Must be True to proceed to D5
        }
        self.disciplines["D4"]["status"] = "complete"
        return self.disciplines["D4"]["outputs"]

    def execute_d5(self, corrective_actions: List[Dict[str, str]]) -> List:
        """
        D5: Select and verify permanent corrective actions.
        Each action must link to a verified root cause from D4.
        """
        for action in corrective_actions:
            if action.get("root_cause_addressed") not in self.disciplines["D4"]["root_causes"]:
                raise ValueError(
                    f"Corrective action '{action['action']}' does not address a verified root cause. "
                    f"Verified root causes: {self.disciplines['D4']['root_causes']}"
                )
        self.disciplines["D5"]["actions"] = corrective_actions
        self.disciplines["D5"]["status"] = "complete"
        return corrective_actions

    def execute_d6(self, implementation_plan: List[Dict[str, str]]) -> List:
        """D6: Implement and validate permanent corrective actions"""
        for action in implementation_plan:
            action["implemented_at"] = datetime.now().isoformat()
            action["validation_status"] = "PENDING"
        self.disciplines["D6"]["actions"] = implementation_plan
        self.disciplines["D6"]["status"] = "complete"
        return implementation_plan

    def execute_d7(self, preventive_actions: List[Dict[str, str]]) -> List:
        """D7: Prevent recurrence across systems, processes, and policies"""
        self.disciplines["D7"]["actions"] = preventive_actions
        self.disciplines["D7"]["status"] = "complete"
        return preventive_actions

    def execute_d8(self, learnings: List[str], recognition: str) -> Dict:
        """D8: Close the 8D project and capture organisational learnings"""
        self.disciplines["D8"]["learnings"] = learnings
        self.disciplines["D8"]["outputs"] = {
            "learnings_captured": len(learnings),
            "recognition": recognition,
            "project_duration_days": (datetime.now() - self.created_at).days,
        }
        self.disciplines["D8"]["status"] = "complete"
        self.completed_at = datetime.now()
        return self.disciplines["D8"]["outputs"]

    def generate_report(self) -> str:
        """Generate a comprehensive 8D report"""
        report = f"""# 8D Problem-Solving Report
**Complaint Reference**: {self.complaint_id}
**Date Opened**: {self.created_at.strftime('%Y-%m-%d')}
**Date Closed**: {self.completed_at.strftime('%Y-%m-%d') if self.completed_at else 'OPEN'}
**Duration**: {(self.completed_at - self.created_at).days if self.completed_at else 'N/A'} days

---
"""
        for d_key, d_data in self.disciplines.items():
            report += f"""
## {d_key}: {d_data['name']}
**Status**: {d_data['status']}
```
{str(d_data.get('outputs', {}))}
```
"""
        return report
```
