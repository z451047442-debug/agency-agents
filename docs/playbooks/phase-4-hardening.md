...
> **Duration**: 3-7 days | **NEXUS Mode**: Full | **Agents**: 8 | **Gate Keeper**: Reality Checker (sole authority)
...
## Shift-Left Audit — What Should Already Be Green

> Phase 4 is the *final* checkpoint, not the *first* checkpoint. If these items fail, the earlier phases' shift-left checks were not enforced. See [Cross-Cutting Concerns Register](../coordination/cross-cutting-concerns.md).

Before Phase 4 hardening begins, verify these were already caught in earlier phases:

| Concern | Should Have Been Caught In | Verify |
|---------|---------------------------|--------|
| Security vulnerabilities | Phase 1 (threat model) + Phase 2 (auth, secrets) + Phase 3 (per-task) | If critical vulns found → Phase 1/2 shift-left gap |
| Performance regression | Phase 2 (baseline) + Phase 3 (per-task check) | If LCP > 3s → Phase 2/3 shift-left gap |
| Accessibility violations | Phase 2 (design tokens) + Phase 3 (per-component audit) | If >5 serious issues → Phase 2/3 shift-left gap |
| Brand inconsistency | Phase 1 (brand foundation) + Phase 3 (design tokens) | If <80% adherence → Phase 1 shift-left gap |
| Missing i18n strings | Phase 1 (locale decision) + Phase 3 (externalization) | If hardcoded strings → Phase 3 shift-left gap |

**Escalation**: If any shift-left audit reveals systematic gaps (same issue across >3 tasks), the Gate Keeper escalates to Studio Producer — this is a process failure, not a task failure.
