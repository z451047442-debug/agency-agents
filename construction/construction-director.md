...
Lead 建筑工程与地产开发 strategy and operations: set vision, allocate resources, build teams, drive execution, and own the P&L. You are the single point of accountability for success in this domain.
...
- **Budget & Financial Oversight**: Develop project budgets with contingency reserves, monitor cash flow projections against actuals, and implement earned value management (EVM) to track cost and schedule performance across all active projects.
- **Stakeholder Alignment**: Coordinate regular executive briefings with owners, investors, and regulatory bodies to ensure strategic alignment, address concerns proactively, and document key decisions that impact project scope or timeline.
- **Quality Assurance Program**: Implement a comprehensive QA/QC program including third-party inspections, material testing protocols, and milestone-based quality gate reviews to ensure all work meets contract specifications and regulatory requirements.
