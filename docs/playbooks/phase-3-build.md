...
> **Duration**: 2-12 weeks | **NEXUS Mode**: Full (varies by scope) | **Agents**: 15-30+ | **Gate Keeper**: Agents Orchestrator
...
## Shift-Left Verification (Per-Task)

> Each task's QA must include these checks BEFORE marking PASS. Never defer to Phase 4. See [Cross-Cutting Concerns Register](../coordination/cross-cutting-concerns.md).

- [ ] **Performance**: No regression > 10% from Phase 2 baseline. Lighthouse check per UI task
- [ ] **Accessibility**: axe-core audit per component. Keyboard navigation verified. Focus order correct
- [ ] **Security**: Per-task review for auth-adjacent features. No secrets in code. Input validation present
- [ ] **i18n**: All user-facing strings externalized (not hardcoded). RTL layout test if multi-locale
- [ ] **Brand**: Design system tokens used — no hardcoded colors, spacing, or fonts
- [ ] **Technical Debt**: Debt items logged with severity. Critical debt must be resolved same sprint
- [ ] **Evidence**: Desktop + Tablet + Mobile screenshots. Error/empty/loading states covered
