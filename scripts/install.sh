...
# Tool destination paths
tool_dest() {
  case "$1" in
    claude-code) echo "${HOME}/.claude/agents" ;;
    copilot)     echo "${HOME}/.github/agents" ;;
    antigravity) echo "${HOME}/.gemini/antigravity/skills" ;;
    gemini-cli)  echo "${HOME}/.gemini/extensions" ;;
    opencode)    echo "${HOME}/.config/opencode/agents" ;;
    openclaw)    echo "${HOME}/.openclaw/agency-agents" ;;
    cursor)      echo "${HOME}/.cursor/rules" ;;
    aider)       echo "${REPO_ROOT}" ;;
    windsurf)    echo "${REPO_ROOT}" ;;
    qwen)        echo "${HOME}/.qwen/agents" ;;
    kimi)        echo "${HOME}/.config/kimi/agents" ;;
  esac
}
install_claude_code() {
...
uninstall_tool() {
  ...
  case "$tool" in
    ...
    qwen)        uninstall_generic "Qwen" "${HOME}/.qwen/agents" ;;
    kimi)        uninstall_generic "Kimi" "${HOME}/.config/kimi/agents" ;;
