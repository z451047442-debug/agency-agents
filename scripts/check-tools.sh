...
# disagree with it:
#   1. ALL_TOOLS in scripts/install/lib.sh     (exact set — every installable tool)
...
# in convert.sh, and an install_<tool> in scripts/install/lib.sh, then run this
# script — it tells you every place that must agree. No deps beyond bash 3.2
...
# 1. tools.json keys == ALL_TOOLS in install/lib.sh (exact, both directions).
all_tools="$(bash_array scripts/install/lib.sh ALL_TOOLS)"
missing="$(comm -23 <(echo "$canon") <(echo "$all_tools"))"
extra="$(comm -13 <(echo "$canon") <(echo "$all_tools"))"
[[ -n "$missing" ]] && fail "scripts/install/lib.sh ALL_TOOLS is missing tool(s) in $JSON: $(echo $missing)"
[[ -n "$extra"   ]] && fail "scripts/install/lib.sh ALL_TOOLS has tool(s) not in $JSON: $(echo $extra)"
...
echo "PASSED: $count tools consistent across $JSON, install/lib.sh, and convert.sh."
