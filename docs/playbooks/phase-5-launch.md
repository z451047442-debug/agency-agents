...
> **Duration**: 2-4 weeks | **NEXUS Mode**: Full (T-7 through T+14) | **Agents**: 16 | **Gate Keepers**: Studio Producer + Analytics Reporter
...
> Pre-launch checks to prevent public-facing issues. See [Cross-Cutting Concerns Register](../coordination/cross-cutting-concerns.md).

- [ ] **Brand**: Launch content brand review — all marketing materials pass Brand Guardian audit
- [ ] **SEO**: Lighthouse SEO score ≥ 90. Meta tags verified. Sitemap submitted
- [ ] **Analytics**: All conversion tracking verified. Funnel events firing correctly
- [ ] **Accessibility**: Public pages pass axe-core audit (zero critical violations before launch)
- [ ] **Performance**: Production load test passed. Auto-scaling verified at 10x baseline

## Rollback Readiness (Mandatory Pre-Flight)

Before any production deployment, the DevOps Automator must complete the [Rollback & Recovery Runbook](../runbooks/scenario-rollback-recovery.md) checklist:

- [ ] Rollback mechanism tested in staging (target < 5 minutes)
- [ ] Database migration rollback tested (if schema changes)
- [ ] Error rate / latency / business metric alerts configured
- [ ] Decision authority defined (P0: DevOps autonomous, P1: Studio Producer)
...
- [ ] Rollback readiness checklist complete (see above)
