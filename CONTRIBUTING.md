...
2. **Choose the appropriate category** (or propose a new one):
   ...
   - `gis/` — Geospatial analysis, cartography, remote sensing, GIS development
...
5. **Test your agent** in real scenarios -- use it in your AI coding tool to verify the prompts produce useful results.
6. **Run the test suite** if you modified any Python scripts: `python -m pytest tests/ -v`. CI will run these tests, so it is best to catch failures early.
7. **Submit a Pull Request** with your agent
...
emoji: 🎯
version: "1.0.0"                       # semver for this agent definition
date_added: "2026-07-05"               # date this agent was first added (YYYY-MM-DD)
...
Not every contribution needs to be a masterpiece. Pick the tier that matches your
time and experience, then level up when you're ready.

### Tier 1: Basic (≈ 20 minutes)

**What you deliver**: One agent `.md` file.

**Requirements**:
- Valid frontmatter (`name`, `description`, `emoji`, `color`)
- Identity, Mission, and Rules sections
- At least 100 words of meaningful content
- Passes `lint-agents.sh` with zero errors
- Passes originality check

**Best for**: First-time contributors, niche specialists, quick gap-fills.

### Tier 2: Standard (≈ 45 minutes)

**What you deliver**: Tier 1 + depth and examples.

**Additional requirements**:
- All recommended sections: Identity, Mission, Rules, Deliverables, Workflow, Success Metrics
- At least 2 concrete code or template examples
- Specific, measurable success metrics (not "better quality" — "reduces page load by 60%")
- Distinct personality and voice

**Best for**: Most agents. This is the quality bar we aim for across the catalog.

### Tier 3: Full (≈ 90 minutes)

**What you deliver**: Tier 2 + ecosystem integration.

**Additional requirements**:
- `nexus_roles` assigned (which NEXUS pipeline phases the agent participates in)
- `depends_on` declared (agent IDs this agent works with, validated against AGENTS.json)
- Tested in at least 2 real scenarios with documented results
- Optional: `vibe` field for personality priming

**Best for**: Agents that participate in multi-agent workflows and platform-defining specialists.
...
Submit at any tier and upgrade later. Many contributors start with Basic to
learn the process, then level up on their next contribution.
...
7. **Run tests for script changes**: If you modified any Python scripts (including `lint-agents.py`, `convert.py`, or i18n tools), run `python -m pytest tests/ -v` to confirm nothing is broken. New script features should include tests.
8. **Proofread**: Check for typos, formatting issues, clarity
