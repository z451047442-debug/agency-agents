...
# Extract depends_on values from a single agent file frontmatter.
# Handles both single-line (depends_on: id1, id2) and YAML list formats.
extract_depends_on() {
  local file="$1"
  local frontmatter deps=()

  frontmatter=$(awk 'NR==1{next} /^---$/{exit} {print}' "$file" 2>/dev/null || true)
  [[ -n "$frontmatter" ]] || return

  # Try YAML list format first: depends_on:\n  - id1\n  - id2
  local dep_list
  ...
  if [[ -n "$dep_list" ]]; then
    while IFS= read -r line; do
      local val
      val=$(echo "$line" | sed 's/^  - //' | sed 's/^"//;s/"$//' | sed "s/^'//;s/'$//" | xargs)
      [[ -n "$val" ]] && deps+=("$val")
    done <<< "$dep_list"
  else
  ...
  # Output JSON entry: "id": ["dep1", "dep2"]
  printf '"%s": %s' "$id" "$(python3 -c "import json; print(json.dumps(['$(printf "'%s'," "${deps[@]}" | sed "s/,$//")']))" 2>/dev/null || echo '[]')"
}

build_depends_manifest() {
  local out="$1"  # output dir (integrations/)
  local manifest="$out/depends_on.json"
  local entries=()
  ...
  if [[ ${#entries[@]} -gt 0 ]]; then
    ...
    echo '}' >> "$manifest"
    info "Depends-on manifest: $manifest ($i agents with dependencies)"
  else
...
main() {
  ...
  local INTS_DIR="$OUT_DIR"  # real integrations/ dir for diffing
  ...
  build_depends_manifest "$OUT_DIR"

  echo ""
  if $check_mode; then
    header "The Agency -- Checking integrations are up to date"
    if diff -rq "$OUT_DIR" "$INTS_DIR" >/dev/null 2>&1; then
      info "OK: integrations/ is up to date."
      exit 0
    else
      error "integrations/ is stale. Run ./scripts/convert.sh and commit the result."
      echo ""
      diff -r "$OUT_DIR" "$INTS_DIR" || true
      exit 1
    fi
  fi
  if $use_parallel && [[ "$tool" == "all" ]]; then
